...
# FONT_NAME should match the primary font used in the website's CSS.
FONT_NAME = "Inter"
...
try:
    ...
    dot_map.attr(rankdir='TB', label='AMC10 Personalized Knowledge Map - Alex', labelloc='t', fontsize='24', fontname=FONT_NAME)
    dot_map.attr('node', shape='box', style='rounded,filled', fontname=FONT_NAME)
    for division in df['Division'].unique():
        with dot_map.subgraph(name=f'cluster_{division}') as c:
            c.attr(label=division, style='rounded', color='lightblue', fontname=FONT_NAME)
